...
func main() {
	...
	hts221 := i2c.NewHTS221Driver(rpiAdaptor)
	bmp280 := i2c.NewBMP280Driver(rpiAdaptor)

	work := func() {
		gobot.Every(sampleInterval, func() {
			hts221Temp, err := hts221.Temperature()
			if err != nil {
				log.Fatalf("failed to read temperature from HTS221: %s", err)
			}
			humidity, err := hts221.Humidity()
			if err != nil {
				log.Fatalf("failed to read humidity from HTS221: %s", err)
			}
			rawPressure, err := bmp280.Pressure()
			if err != nil {
				log.Fatalf("failed to read pressure from BMP280: %s", err)
			}

			tempF := DegreesCToF(hts221Temp)
			dewPointF := DewPointF(tempF, humidity)
			indoorHumidityRec := IndoorHumidityRecommendation(tempF)
			pressure := MSLP(float64(rawPressure), *elevation)
			...
			point := influxdb2.NewPoint(
				...
				map[string]interface{}{
					...
					"temperature_c":                   hts221Temp,
					"humidity":                        humidity,
					"raw_pressure_pa":                 rawPressure,
